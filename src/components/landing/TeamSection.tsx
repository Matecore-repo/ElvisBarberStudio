'use client';

import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Scissors, Star } from "lucide-react";
import Link from "next/link";
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';

interface Barber {
  id: string;
  name: string;
  commissionRateDefault: number;
  active: boolean;
}

interface TeamSectionProps {
  staff: Barber[];
}

// Imágenes de barberos placeholder
const barberImages = [
  "https://images.unsplash.com/photo-1621605815971-fbc98d665033?w=800&h=600&fit=crop",
  "https://images.unsplash.com/photo-1622287162716-f311baa1a2b8?w=800&h=600&fit=crop",
  "https://images.unsplash.com/photo-1598971639058-fab3c3109a00?w=800&h=600&fit=crop",
  "https://images.unsplash.com/photo-1633681926022-84c23e8cb2d6?w=800&h=600&fit=crop",
  "https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=800&h=600&fit=crop",
  "https://images.unsplash.com/photo-1585747860715-2ba37e788b70?w=800&h=600&fit=crop",
];

interface BarberCardProps {
  barber: Barber;
  index: number;
  hoveredIndex: number | null;
  setHoveredIndex: (index: number | null) => void;
}

function BarberCard({ barber, index, hoveredIndex, setHoveredIndex }: BarberCardProps) {
  const image = barberImages[index % barberImages.length];

  return (
    <motion.article
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ delay: index * 0.1, duration: 0.5, ease: "easeOut" }}
      onHoverStart={() => setHoveredIndex(index)}
      onHoverEnd={() => setHoveredIndex(null)}
      className="relative group overflow-hidden rounded-2xl h-96 cursor-pointer transition-all duration-300 ease-out hover:scale-[1.02]"
      style={{
        boxShadow: hoveredIndex === index
          ? "0 0 30px rgba(212, 175, 55, 0.4)"
          : "0 10px 30px rgba(0, 0, 0, 0.5)",
      }}
    >
      {/* Hover background glow */}
      <AnimatePresence>
        {hoveredIndex === index && (
          <motion.div
            className="absolute inset-0 bg-amber-500/20 z-[1]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1, transition: { duration: 0.3 } }}
            exit={{ opacity: 0, transition: { duration: 0.3 } }}
          />
        )}
      </AnimatePresence>

      {/* Image with zoom hover */}
      <div className="absolute inset-0 z-0">
        <img
          src={image}
          alt={`${barber.name} - Barbero profesional`}
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
        />
      </div>

      {/* Overlay gradient for readability */}
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent z-[2]" />

      {/* Status Badge */}
      <div className="absolute top-4 right-4 z-10">
        <Badge
          variant={barber.active ? 'default' : 'secondary'}
          className={barber.active
            ? 'bg-green-600/80 text-green-100 border-green-500/50 backdrop-blur-sm'
            : 'bg-gray-600/80 text-gray-300 border-gray-500/50 backdrop-blur-sm'
          }
        >
          {barber.active ? '● Disponible' : 'No disponible'}
        </Badge>
      </div>

      {/* Content */}
      <div className="relative z-10 h-full flex flex-col justify-end p-6">
        <motion.div
          className="flex items-center gap-2 mb-2"
          initial={{ y: 10, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: index * 0.1 + 0.2 }}
        >
          <Scissors className="w-5 h-5 text-amber-400" />
          <span className="text-sm text-amber-400 font-medium">Barbero Profesional</span>
        </motion.div>

        <motion.h3
          className="text-2xl font-bold text-white mb-2 tracking-tight"
          initial={{ y: 10, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: index * 0.1 + 0.3 }}
        >
          {barber.name}
        </motion.h3>

        <motion.div
          className="flex items-center gap-4 text-sm text-gray-400 mb-4"
          initial={{ y: 10, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: index * 0.1 + 0.4 }}
        >
          <div className="flex items-center gap-1">
            <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
            <span>4.9</span>
          </div>
          <span>•</span>
          <span>Cortes & Barba</span>
        </motion.div>

        {/* CTA Button */}
        <motion.div
          initial={{ y: 10, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: index * 0.1 + 0.5 }}
        >
          <Link href="/login">
            <Button
              disabled={!barber.active}
              className="w-full gold-metallic-button disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {barber.active ? `Agendar con ${barber.name.split(' ')[0]}` : 'No disponible'}
            </Button>
          </Link>
        </motion.div>
      </div>

      {/* Golden border hover effect */}
      <div
        className="absolute inset-0 border-2 rounded-2xl transition-all duration-300 pointer-events-none z-[3]"
        style={{
          borderColor: hoveredIndex === index ? "#D4AF37" : "transparent",
        }}
      />

      {/* Subtle noise texture overlay */}
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-30 mix-blend-overlay z-[2]" />
    </motion.article>
  );
}

export const TeamSection = ({ staff }: TeamSectionProps) => {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  return (
    <section id="barberos" className="relative py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-black via-neutral-950 to-black">
      <div className="max-w-7xl mx-auto">
        {/* Header Section */}
        <motion.div
          className="mb-12 text-center"
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
            Nuestros <span className="gold-gradient-text">Barberos</span>
          </h2>
          <p className="text-lg text-gray-400 max-w-2xl mx-auto">
            Profesionales certificados con años de experiencia en cortes modernos y técnicas clásicas
          </p>
        </motion.div>

        {/* Grid of Barber Cards */}
        {staff.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {staff.map((barber, idx) => (
              <BarberCard
                key={barber.id}
                barber={barber}
                index={idx}
                hoveredIndex={hoveredIndex}
                setHoveredIndex={setHoveredIndex}
              />
            ))}
          </div>
        ) : (
          <div className="text-center py-12 text-gray-500 border border-neutral-800 rounded-xl bg-neutral-900/50">
            <Scissors className="w-12 h-12 mx-auto mb-4 text-gray-600" />
            <p className="text-lg">No hay barberos disponibles en este momento</p>
          </div>
        )}
      </div>
    </section>
  );
};
